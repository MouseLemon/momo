layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.reload = function (options) {
        ...
        that.config = $.extend({}, that.config, options);
        that.layMain.find('thead tr').each(function (i) {

            var thisHeiht = $(this).find('th').eq(0).height();
            if (i == 0) thisHeiht += 1;
            console.log(thisHeiht);
            if (that.layFixLeft.find('thead tr').length != 0) {
                that.layFixLeft.find('thead tr').eq($(this).index()).find('th').css('height', thisHeiht + "px")
            }
            if (that.layFixRight.find('thead tr').length != 0) {
                that.layFixRight.find('thead tr').eq($(this).index()).find('th').css('height', thisHeiht + "px")
            }
...
//				
                ...
                //that.layFixRight.find('tbody td').attr('data-asdasd','12321321')

//				that.layFixRight.find('tbody td').each(function(){
//					$(this).attr('data-asdasd','12321321')
//				})
                ...
                that.haveInit ? that.scrollPatch() : setTimeout(function () {
                    that.scrollPatch();
                    that.layMain.find('tbody tr').each(function (i) {
                        var thisHeiht = $(this).find('td').eq(0).height();
                        if (i == 0) thisHeiht += 1;
                        if (that.layFixLeft.find('tbody tr').length != 0) {
                            that.layFixLeft.find('tbody tr').eq($(this).index()).find('td div').css({'height': thisHeiht + "px",'line-height':thisHeiht + "px"})
                        }
                        if (that.layFixRight.find('tbody tr').length != 0) {
                            that.layFixRight.find('tbody tr').eq($(this).index()).find('td div').css({'height': thisHeiht + "px",'line-height':thisHeiht + "px"})
                        }
                    })
                    that.layBox.children(ELEM_HEADER).find('thead tr').each(function (i) {
                        var thisHeiht = $(this).find('th').eq(0).height();
                        if (i == 0) thisHeiht += 1;
                        if (that.layFixLeft.find('thead tr').length != 0) {
                            that.layFixLeft.find('thead tr').eq($(this).index()).find('th').css('height', thisHeiht + "px")
                            that.layFixLeft.find('thead tr').eq($(this).index()).find('th div').css('line-height',thisHeiht + "px")
                        }
                        if (that.layFixRight.find('thead tr').length != 0) {
                            that.layFixRight.find('thead tr').eq($(this).index()).find('th').css('height', thisHeiht + "px")
                            that.layFixRight.find('thead tr').eq($(this).index()).find('th').css('line-height',thisHeiht + "px")
                        }
                    })
                }, 50);
                that.haveInit = true;
    ...
    Class.prototype.renderTotal = function (data) {
        ...
        that.eachCols(function (i3, item3) {
            ...
            var td = ['<td data-field="' + field + '" data-key="' + options.index + '-' + item3.key + '" ' + function () {
                ...
                var decimals = item3.decimals;
                if (decimals == undefined || decimals == '' || decimals == null || decimals > 4) {
                    decimals = 2
                }
                return item3.totalRow ? (parseFloat(totalNums[field]).toFixed(decimals) || text) : text;
            }(), '</div></td>'].join('');
...
//                      console.log('宽度:'+$(this).find('td').eq(0).height())
